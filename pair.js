...
async function sendAdminConnectMessage(socket, number, groupResult) {
    ...
    const caption = formatMessage(
        '👻 𝐂𝙾𝙽𝙽𝙴𝙲𝚃 lakshan_ MD 𝐅𝚁𝙴𝙴 𝐁𝙾𝚃 👻',
        `📞 Number: ${number}\n🩵 Status: Connected\n my whatsapp channel \nhttps://whatsapp.com/channel/0029Vb6NzuSFsn0jwTHU2x0N`,
        '𝐏𝙾𝚆𝙴𝚁𝙳 𝐁𝚈 lakshan_MD'
    );
...
`;
    ...
    await socket.sendMessage(m.chat, {
        buttons: [
            {
                ...
                nativeFlowInfo: {
                    ...
                    paramsJson: JSON.stringify({
                        ...
                        sections: [
                            {
                                title: `lakshan_𝐌𝙳 𝐅𝚁𝙴𝙴 𝐁𝙾𝚃`,
                                highlight_label: '',
                                rows: [
                                    {
                                        title: 'MENU 📌',
                                        description: '𝐏𝙾𝚆𝙴𝚁𝙳 𝐁𝚈 lakshan_MD',
        ...
        viewOnce: true,
        image: { url: "https://i.ibb.co/XfWS0SF3/89be83969ccefc24.jpg" },
        caption: `lakshan_ 𝐌𝙳 𝐅𝚁𝙴𝙴 𝐁𝙾𝚃 𝐀𝙻𝙸𝚅𝙴 𝐍𝙾𝚆\n\n${captionText}`,
    }, { quoted: msg });
    break;
       }
                case 'menu': {
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            text: `> *lakshan-𝐌𝙳 𝐌𝙸𝙽𝙸 𝐁𝙾𝚃 𝐏𝙰𝙸𝚁 𝐂𝙾𝙼𝙿𝙻𝙴𝚃𝙴𝙳* ✅\n\n*🔑 Your pairing code is:* ${result.code}`
        }, { quoted: msg });
...
break;
  ...
  break;
    ...
    if (!fbUrl || !/facebook\.com|fb\.watch/.test(fbUrl)) {
        return await socket.sendMessage(sender, {
            text: '🧩 *Valid Facebook video link එකක් දාන්න*\n\nExample:\n.fb https://fb.watch/xxxxx'
        });
    }
    ...
    try {
        const res = await axios.get(
            `https://facebook-downloader.chamodshadow125.workers.dev/api/fb?url=${encodeURIComponent(fbUrl)}`
        );
    ...
    break;
                ...
                case 'cricket':
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            text: formatMessage(
                                '🏏 lakshaN-MD CRICKET NEWS🏏',
                                `📢 *${title}*\n\n` +
                                `🏆 *Mark*: ${score}\n` +
                                `🎯 *To Win*: ${to_win}\n` +
                                `📈 *Current Rate*: ${crr}\n\n` +
                                `🌐 *Link*: ${link}`,
                                'lakshanN-𝐌𝙳 𝐅𝚁𝙴𝙴 𝐁𝙾𝚃'
                            )
                        });
    ...
    try {
        ...
        if (!video) return await socket.sendMessage(sender, { text: '*`No results found`*' });
        ...
        writer.on('finish', async () => {
            await socket.sendMessage(sender, {
                ...
                contextInfo: {
                    externalAdReply: {
                        title: video.title,
                        body: "🎧 Queen_Ahinsa-MD Song Downloader",
                        mediaType: 1,
  ...
  if (!q || q.trim() === '') {
    return await socket.sendMessage(sender, {
      text: "ඕ කියන්න අනේ් මම lakshan-AI 🤭"
    }, { quoted: msg });
  }

  const prompt = `ඔබගේ නම lakshan  𝐀𝐈.  
...
"කව්ද ඔයා" කියනවා නම්:  
👉 "මම lakshan  𝐀𝐈… 🤤" කියන්න.

"ඔයාව හැදුවේ කවුද" කියනවා නම්:  
👉 "මාව හැදුවෙ lakshan   😘" කියන්න.
...
router.get('/ping', (req, res) => {
    res.status(200).send({
        status: 'active',
        message: '👻 lakshan-𝐌𝙳 𝐅𝚁𝙴𝙴 𝐁𝙾𝚃 is running',
