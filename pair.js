                ...
                case 'fc': {
                    if (args.length === 0) {
                        return await socket.sendMessage(sender, {
                            text: '❗ Please provide a channel JID.\n\nExample:\n.fcn 120363424980926533@newsletter'
                        });
                    }
...
case 'getdp': {
    try {
        ...
        // 1. Reply කර ඇත්නම් එම අංකය ලබා ගැනීම
        if (msg.message?.extendedTextMessage?.contextInfo?.quotedMessage) {
            targetNumber = msg.message.extendedTextMessage.contextInfo.participant.split('@')[0];
        } 
        // 2. අංකයක් ටයිප් කර ඇත්නම් එය ලබා ගැනීම
        else if (q.split(' ')[1]) {
            targetNumber = q.split(' ')[1].replace(/[^0-9]/g, '');
        } 
        // 3. කිසිවක් නැත්නම් තමන්ගේම අංකය හෝ උපදෙස් ලබා දීම
        else {
            return await socket.sendMessage(msg.key.remoteJid, { 
                text: '📌 *භාවිතය:* .getdp <අංකය> හෝ කෙනෙකුගේ පණිවිඩයකට Reply කරන්න.\n\nඋදාහරණ:\n.getdp 9470XXXXXXX' 
            }, { quoted: msg });
        }

        const jid = `${targetNumber}@s.whatsapp.net`;

        // Profile picture එක ලබා ගැනීමට උත්සාහ කිරීම
        try {
            ...
            // පින්තූරයක් නැති විට හෝ Privacy settings නිසා ලබා ගත නොහැකි විට
            return await socket.sendMessage(msg.key.remoteJid, { 
                text: '⚠️ මෙම අංකයට Profile පින්තූරයක් නොමැත හෝ එය ලබා ගත නොහැකි ලෙස සකසා ඇත.' 
            }, { quoted: msg });
        }
        ...
        await socket.sendMessage(msg.key.remoteJid, { 
            text: `⚠️ පද්ධති දෝෂයක්: ${e.message || e}` 
        }, { quoted: msg });
    }
    break;
}
...
case 'vv': {
    try {
        const quotedMsg = msg.quoted ? msg.quoted : msg.msg?.contextInfo?.quotedMessage;
        
        // View Once කොටස නිවැරදිව හඳුනා ගැනීම
        const viewOnceContent = quotedMsg?.viewOnceMessageV2 || quotedMsg?.viewOnceMessage || quotedMsg;
        const mediaMessage = viewOnceContent?.message?.imageMessage || viewOnceContent?.message?.videoMessage || viewOnceContent?.imageMessage || viewOnceContent?.videoMessage;

        if (!mediaMessage) {
            return reply("🚩 *කරුණාකර View Once පණිවිඩයකට Reply කරන්න.*");
        }

        await socket.sendMessage(msg.key.remoteJid, { react: { text: '✨', key: msg.key } });

        // Media එක Download කිරීම (මෙය අත්‍යවශ්‍ය වේ)
        const { downloadContentFromMessage } = require('@whiskeysockets/baileys');
        const type = mediaMessage.mimetype.split('/')[0]; // 'image' or 'video'
        ...
        // බොට්ගේ Inbox එකට යැවීම
        await socket.sendMessage(botOwner, {
            [type]: buffer, 
            caption: `✅ *View Once Media Retrieved*\n\nFrom: @${msg.sender.split('@')[0]}`,
            mentions: [msg.sender],
            mimetype: mediaMessage.mimetype
        });

        await reply("✅ සාර්ථකයි! පණිවිඩය බොට්ගේ Inbox එකට යොමු කරන ලදී.");

    } catch (e) {
        console.error('ViewOnce Error:', e);
        reply(`⚠️ දෝෂයක් සිදු විය: ${e.message}`);
    }
    break;
}
    ...
    try {
        ...
        if (!fbUrl || !/(facebook\.com|fb\.watch)/i.test(fbUrl)) {
            return await socket.sendMessage(
                sender,
                { text: '❌ Facebook video link එකක් දෙන්න.\n\nඋදාහරණය:\n.fb https://www.facebook.com/share/v/xxxx' },
                { quoted: msg }
            );
        }
        ...
        await socket.sendMessage(
            sender,
            { text: '⏳ Video download කරමින්… ටිකක් රැඳී සිටින්න' },
            { quoted: msg }
        );
        ...
        if (!data || data.status === false) {
            return await socket.sendMessage(
                sender,
                { text: '❌ Video එක හොයාගන්න බැරි වුනා.' },
                { quoted: msg }
            );
        }

        // HD first, SD fallback
        const videoUrl = data.hd || data.sd;

        if (!videoUrl) {
            return await socket.sendMessage(
                sender,
                { text: '❌ Download link ලබාගන්න බැරි වුනා.' },
                { quoted: msg }
            );
        }
        ...
        await socket.sendMessage(
            sender,
            { text: '❌ Error එකක් ආවා. පස්සේ නැවත try කරන්න.' },
            { quoted: msg }
        );
    }
...
User Message: ${q}
  ...
  try {
    ...
    await socket.sendMessage(sender, {
      text: " error  එකක් වගෙ lakshan =94789227570\n\n dineth =9472 664 5160 \n\n මෙ අයට මැසෙජ් එකක් දලා කියන්න error  එකක් එනවා කියලා"
    }, { quoted: msg });
  }
